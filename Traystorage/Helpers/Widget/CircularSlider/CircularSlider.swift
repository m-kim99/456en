...
//
    ...
    func drawLine(_ ctx: CGContext) {
        ...
        */
        CircularTrig.drawUnfilledGradientArcInContext(ctx, center: centerPoint, radius: computedRadius, lineWidth: CGFloat(lineWidth), maximumAngle: CGFloat(angleFromNorth), colors: [unfilledColor, filledColor], lineCap: filledArcLineCap)
    }
    ...
    func drawInnerLabels(_ ctx: CGContext, rect: CGRect) {
        if let labels = innerMarkingLabels, labels.count > 0 {
            let attributes = [.font: labelFont, .foregroundColor: labelColor] as [NSAttributedString.Key: Any]
    ...
    override func endTracking(_ touch: UITouch?, with event: UIEvent?) {
        sendActions(for: UIControl.Event.touchDragOutside)
    }
